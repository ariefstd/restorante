...
class Membership_model extends CI_Model
{
	...
	function get_newrenewer($start = "")
	{
		...
		if($start != "")
			...
			//$query 	= $this->db->query("SELECT * FROM membership WHERE id>=582 AND id<=721 ORDER BY id DESC LIMIT $start,$range");
		else
			...
			//$query 	= $this->db->query("SELECT * FROM membership WHERE id>=582 AND id<=721 ORDER BY id DESC");
