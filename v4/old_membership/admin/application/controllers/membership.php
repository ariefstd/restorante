...
class Membership extends CI_Controller {
	...
	{
		$res = $this->db->query("SELECT * FROM temp")->result();
		foreach($res as $r){
			$this->db->query("UPDATE membership SET address = ? WHERE member_card_no=?",array($r->address,$r->nhk));
		}
	}
	*/
	...
	public function print_post($type,$page)
	{
		...
		foreach ($result->result_array() as $resultdata){
			...
			if($type=="birthday"){
				...
				$this->fpdf_lib->Cell(28,1,'',0);
				$this->fpdf_lib->SetFont('Big5','',9);
				$this->fpdf_lib->MultiCell(142,4,$this->ncr_decode('衷心的感謝您這幾年來對新香港酒樓不間斷的支持和鼓勵，為表謝意，特此獻上您的貴賓卡（貴賓卡號碼: '.$resultdata["member_card_no"].'.）. 此貴賓卡將從'.date("d/m/Y",$resultdata["date_join"]).'起生效，有效期為一年。我們也準備了價值RM20的優惠券給您享用'));	
				$this->fpdf_lib->Ln(2);
				$this->fpdf_lib->Cell(28,1,'',0);
				$this->fpdf_lib->SetFont('Big5','',9);
