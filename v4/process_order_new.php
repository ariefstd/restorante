...
if(empty($check))
{
	...
	$result=mysqli_query($con,$sql);		$client_id=mysqli_insert_id($con);
}
else
{
...
if($result)
{
	...
	if(!empty($email))
	{
		...
		$body = str_replace('%%MESSAGE%%', $order['order_message'], $body ) ;
                $body = str_replace('%%AVAILABLE%%', $available, $body ) ;
		...
		$mail->AddCC($order['client_email'], $man_name);
